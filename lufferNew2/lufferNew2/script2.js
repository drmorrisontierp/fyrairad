...
function customConfirm(text) {
  ...
  function winConfirm(_use, text, _player) {
    return new Promise((resolve, reject) => {
      console.log(_player)
      let symbol = [];
      symbol[0] = _player ? "X" : "O";
      symbol[1] = _player ? "O" : "X";
      const mainDiv = document.createElement("div")
      const textDiv = document.createElement("div")
      const buttonDiv = document.createElement("div")
      const warningDiv = document.createElement("div")
     ...
     <text
        ...
        xml:space="preserve"><tspan
          style="fill:#ffffff;stroke-width:0.53682709"
          y="97.603447"
          x="60.391994"
          id="tspan3736">${symbol[1]}</tspan></text>
     <text
        ...
        xml:space="preserve"><tspan
          style="fill:#ffffff;stroke-width:0.61848068"
          y="98.435966"
          x="143.74713"
          id="tspan3740">${symbol[0]}</tspan></text>
                          `
      ...
      _cancelButton.focus()
      textDiv.innerHTML = text;
      if (_use == "win") {setAttributes(warning, {
        "viewBox": "0 0 200 200",
        "height": "100%",
        "width": "100%",
...
const ui = {
    confirm: async (message) => customConfirm(message),
    win: async (message, player) => winConfirm(message, player),
  }

async function confirmWin(player, games = 1) {
    let winner = player ? playerNames[0] : playerNames[1];
    console.log(player)
    const confirm = await ui.win(`Congratulations ${winner} on a fine win. Shall we make it best out of ${games}`, player);
    if (confirm) {
      reset(2);
    } else {
      reset(0);
...
async function confirmReset() {
  pauseTimer(true)
  changeClick(false)
  const confirm = await ui.confirm("Are you sure you want to reset");
  if (confirm) {
    reset(1);
  } else {
    changeClick(true)
...
async function confirmChoice(choice) {
  ...
  const confirm = await ui.confirm(_text);
  if (confirm && _flag) {
     start(choice);
  } else {
    return 0;
...
function generateAllGridNumbers(_numbers) {
  ...
  if (gameMode === "negative") {
   // console.log("negative")
    if (options === 1) {
      //console.log("negative O1")
      operators = ["+", "-"]
      digits = 3;
    } else if (options === 2) {
     // console.log("negative O2")
      operators = ["+", "-"]
      digits = 5;
    } else if (options === 3) {
      operators = ["*", "/"]
